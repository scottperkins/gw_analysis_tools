...
//check that these are coming out the way I expect them to...
...
//equation 19
n_NRT[0] = c_NRT[0] + d_NRT[0];
n_NRT[1] = ((c_NRT[0] * c_NRT[1]) - c_NRT[3] - (c_NRT[1] * d_NRT[0]) + n_NRT[3])/c_NRT[0];
n_NRT[2] = c_NRT[2] + (c_NRT[0] * d_NRT[0]) + d_NRT[2];
d_NRT[1] = -(c_NRT[3] + (c_NRT[1] * d_NRT[0]) - n_NRT[3])/c_NRT[0];
*/
