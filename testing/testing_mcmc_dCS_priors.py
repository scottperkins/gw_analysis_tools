...
burn = False
data = np.loadtxt("data/mcmc_output_dCS_priors.csv",delimiter=',')
if burn:
    data = data[10000:]
...
plt.savefig("mcmc_testing_dCS_priors.pdf")
plt.close()
alphahist = []
for x in dataplot:
...
plt.savefig("alpha_hist_dCS_priors.pdf")
plt.close()
##############################################################
data = np.loadtxt("data/mcmc_output_dCS_hot_priors.csv",delimiter=',')
if burn:
    data = data[10000:]
#for i in np.arange(9):
#    parameter = [x[i] for x in data]
...
for x in data:
    ...
    dataplot[-1][-1]=(dataplot[-1][-1])**(1./4.)*(3e5)
figure = corner.corner(dataplot, labels=labels,quantiles=[.16,.5,.84], show_titles=True)
plt.savefig("mcmc_testing_dCS_hot_priors.pdf")
plt.close()

##############################################################
autocorr = np.loadtxt("data/auto_corr_mcmc_dCS_priors.csv",delimiter=',')
lengths = autocorr[0]
autocorr = autocorr[1:]
...
plt.legend()
plt.savefig("autocorr_testing_dCS_priors.pdf")
plt.close()
...
plt.savefig("mcmc_testing_transform_dCS_priors.pdf")
plt.close()
