...
plt.savefig("mcmc_testing_ppE.pdf")
plt.close()
betahist = []
for x in dataplot:
    betahist.append(x[-1])
...
plt.hist(betahist,bins=100,density=True)
plt.savefig("beta_hist_ppE.pdf")
plt.close()
##############################################################
data = np.loadtxt("data/mcmc_output_ppE_hot.csv",delimiter=',')
if burn:
    data = data[10000:]
#for i in np.arange(9):
#    parameter = [x[i] for x in data]
...
for x in data:
    ...
    #dataplot[-1][-1]=(dataplot[-1][-1])**(1./4.)*(3e5)
figure = corner.corner(dataplot, labels=labels,quantiles=[.16,.5,.84], show_titles=True)
plt.savefig("mcmc_testing_ppE_hot.pdf")
plt.close()

##############################################################
autocorr = np.loadtxt("data/auto_corr_mcmc_ppE.csv",delimiter=',')
lengths = autocorr[0]
autocorr = autocorr[1:]
...
plt.legend()
plt.savefig("autocorr_testing_ppE.pdf")
plt.close()
...
plt.savefig("mcmc_testing_transform_ppE.pdf")
plt.close()
