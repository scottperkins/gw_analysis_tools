...
data = np.loadtxt("data/snr_comp_MBH.csv",delimiter=',',unpack=True)
#labels = ["SIMPS","GL_NONLOG","GL_LOG"]
#labels = ["GL_NONLOG","GL_LOG"]
labels = ["GL_NONLOG","GL_LOG","GL_NONLOG_N","GL_LOG_N"]

temp= []
#temp.append(np.abs((data[0]-data[1])/data[0]))
temp.append(np.abs((data[0]-data[2])/data[0]))
temp.append(np.abs((data[0]-data[3])/data[0]))
temp.append(np.abs((data[0]-data[8])/data[0]))
temp.append(np.abs((data[0]-data[9])/data[0]))
resids = []
...
ax[1].legend()
ax[1].set_xscale('log')
plt.savefig("plots/snr_comp_MBH.pdf")
plt.close()
